...
# Carregar a imagem
imagem = Image.open('ead.png')  # Substitua 'ead.png' pelo nome ou caminho da sua imagem
...
if total > 0:
    ...
    st.info("Após realizar o pagamento, marque a confirmação abaixo para enviar o pedido.")

    # Confirmação de pagamento
    confirmacao_pagamento = st.checkbox("Eu confirmo que realizei o pagamento via Pix.")
    ...
    with st.form("form_pedido"):
        ...
        # Botão de envio só habilitado se o pagamento for confirmado
        submitted = st.form_submit_button("Enviar Pedido", disabled=not confirmacao_pagamento)
    ...
    if submitted:
        ...
        # Função para enviar os dados para o Google Forms
        def enviar_para_formulario_google(dados_pedido):
            # Acessar o form_id e entry IDs dos secrets
            form_id = st.secrets["google_forms"]["form_id"]
            url_formulario = f'https://docs.google.com/forms/d/e/{form_id}/formResponse'

            payload = {
                f'entry.{st.secrets["google_forms"]["entry_pedido_id"]}': dados_pedido['Pedido ID'],
                f'entry.{st.secrets["google_forms"]["entry_data"]}': dados_pedido['Data'],
                f'entry.{st.secrets["google_forms"]["entry_nome"]}': dados_pedido['Nome'],
                f'entry.{st.secrets["google_forms"]["entry_email"]}': dados_pedido['Email'],
                f'entry.{st.secrets["google_forms"]["entry_telefone"]}': dados_pedido['Telefone'],
                f'entry.{st.secrets["google_forms"]["entry_preco_total"]}': str(dados_pedido['Preço Total']),
                f'entry.{st.secrets["google_forms"]["entry_status"]}': dados_pedido['Status'],
                f'entry.{st.secrets["google_forms"]["entry_observacoes"]}': dados_pedido['Observações'],
                # Adicione outros campos e seus 'entry IDs' correspondentes
            }

            response = requests.post(url_formulario, data=payload)
            if response.status_code == 200 or response.status_code == 302:
                st.info("Dados enviados para o Google Forms com sucesso.")
            else:
                st.error(f"Ocorreu um erro ao enviar os dados para o Google Forms. Código de status: {response.status_code}")

        # Enviar os dados para o Google Forms
        enviar_para_formulario_google(dados_pedido)
    ...
    st.warning("Selecione pelo menos um serviço para continuar.")
